...
namespace PRBD_Framework {
    ...
    public abstract partial class ApplicationBase : Application {
        ...
        public static readonly string IMAGE_PATH = Path.GetFullPath(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location) + "/../../../Image");

        /*public static string IMAGE_PATH {
            get {
                // vérifie d'abord si on trouve un dossier images à côté de l'exécutable
                var path = Path.GetFullPath(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location) + "/Image");
                if (!Directory.Exists(path)) {
                    ...
                    foreach (var frame in trace.GetFrames()) {
                        path = Path.GetFullPath(Path.GetDirectoryName(frame.GetFileName()) + "/Image");
                        if (Directory.Exists(path))
                            return path;
                    }
                    path = null;
